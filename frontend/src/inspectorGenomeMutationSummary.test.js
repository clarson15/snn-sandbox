...
describe('deriveInspectorGenomeMutationSummaryModel', () => {
  it('is hidden for generation-0 organisms with no parent id', () => {
    const selected = {
      id: 'founder-1',
      generation: 0,
      traits: { size: 1, speed: 1, adolescenceAge: 40, visionRange: 1, turnRate: 1, metabolism: 1 },
  ...
  it('returns deterministic sorted changes and remains stable under array permutation', () => {
    const parent = {
      id: 'parent-1',
      traits: { size: 1, speed: 1, adolescenceAge: 30, visionRange: 10, turnRate: 0.1, metabolism: 0.05 },
      brain: {
        neurons: [
    ...
    const selected = {
      ...
      traits: { size: 1.2, speed: 1, adolescenceAge: 45, visionRange: 9, turnRate: 0.2, metabolism: 0.05 },
      brain: {
        neurons: [
    ...
    expect(modelA.traitDeltas.map((item) => item.key)).toEqual(['size', 'adolescenceAge', 'visionRange', 'turnRate']);
