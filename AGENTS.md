...
- `cd frontend && npm run benchmark`: run deterministic performance benchmarks.
...
## Testing Guidelines
Frontend tests use Vitest with Testing Library; backend tests use xUnit with `Microsoft.AspNetCore.Mvc.Testing`. Name tests after observable behavior, for example `replaySummary.test.js` or `Save_RejectsMissingName`. Add or update tests with every simulation, replay, persistence, or API behavior change. For deterministic engine work, cover same-seed replay behavior and benchmark-sensitive paths when relevant.
